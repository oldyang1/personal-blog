...
class Post(models.Model):
    ...
    pv = models.IntegerField(default=0,verbose_name="浏览量")  # 浏览量热门推荐
...
class SideBar(models.Model):
    ...
    title = models.CharField(max_length=50, verbose_name="模块名称")   # 模块的名称
    ...
    content = models.CharField(max_length=500, blank=True, default='', verbose_name="内容",
                               help_text="如果设置的不是HTML类型，可为空")   # 这个字段专门给HTML用，其他类型可为空
    ...
    @classmethod   # 类方法装饰器， 变成这个类的方法可以调用
    def get_sidebar(cls):
        return cls.objects.filter(status=2)  # 查询到所有允许展示的模块


    @property   # 成为一个类属性 ，调用的时候不需要加（）， 是只读的用户没办法修改
    def get_content(self):
        if self.display_type == 1:  # 搜索框
            context = {
            ...
            return self.content   # 在侧边栏直接渲染这里的HTML，模板中必须使用safe过滤器去渲染HTML
